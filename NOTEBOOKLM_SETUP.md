...
**現在の状態（2026-09-20時点）: 方式A・方式Bともに動作確認済み。方式B（VPS完全自動化、PCの電源不要）は、実際の分析セッション（「分析」環境）上で認証切れ→VPS経由の自動再認証→分析続行、という一連の流れが実際に動作することまで確認済み。携帯だけで完結する運用が実現した。**
...
### 4つ目の真因: `session-start.sh`が毎回パッチ版nlmを無印版に上書きしていた

上記3つの真因をすべて修正し、このセッション（開発用ブランチのセッション）からはVPS経由の認証情報取得に成功したにもかかわらず、実際の「分析」環境の新しいセッションでは同じ手順が3回とも同一のTLS証明書エラー（IPアドレスに対する検証失敗）で失敗し続けた。

原因は`.claude/hooks/session-start.sh`自体のバグだった。`GOBIN`（`go install`されたバイナリの場所）を`PATH`に反映する処理が、`command -v nlm`によるnlm存在チェックより**後ろ**に書かれていた。そのため、スクリプト自身がその場で実行される瞬間にはまだ`PATH`に`GOBIN`が含まれておらず、たとえ過去のセッションで既にVPS対応パッチ版nlmをビルド済みで、GOPATH配下のファイル自体はセッションをまたいで永続化されていたとしても、**新しいセッションが始まるたびに「nlmが見つからない」と誤判定し、`go install github.com/tmc/nlm/cmd/nlm@latest`で無印（パッチ未適用）版を上書きインストールしてしまっていた**。その直後のパッチ適用済みマーカーファイルのチェックは、マーカー自体は上書きされず残っていたため「パッチ済み」と誤認し、再ビルドをスキップ——結果として、パッチ版nlmが一見ビルドされているように見えて、実際には毎回無印版に差し替わっている、という状態になっていた。

**発見の決め手**: 「分析」環境で`nlm`バイナリの更新日時とパッチ適用マーカーの更新日時を比較したところ、**nlm本体の方がマーカーより新しい**という矛盾が見つかった（本来はビルド成功→マーカー作成の順なので、マーカーの方が新しいはず）。この矛盾から、ビルドの後に何かがnlmを上書きしていると特定できた。

**対処**: `GOBIN`の`PATH`反映処理を`nlm`存在チェックより前に移動し、さらに二重の安全策として「パッチ適用済みマーカーが存在する場合はnlmの存在チェック自体もスキップする」ロジックを追加（`.claude/hooks/session-start.sh`、mainにマージ済み）。あわせて、既存の壊れた状態（無印nlm＋古いマーカー）を持つ環境では、`rm -f "$(go env GOPATH)/bin/nlm-vps-patched.marker"`でマーカーを手動削除し、次のセッション開始時に正しく再ビルドさせる必要があった。

この修正後、実際の「分析」環境の新しいセッションでNotebookLM認証切れ→手順A（VPS経由の自動再認証）→分析続行、という一連の流れが**PCなしで**成功することを実証した。
